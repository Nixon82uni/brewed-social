...
function buildCard(r, author, likeCount, isLiked, saveCount, isSaved) {
  ...
  card.querySelector('.card-save-btn').addEventListener('click', async e => {
    ...
    try {
      ...
      if (icon) icon.className = `ph ${res.saved ? 'ph-bookmark-simple-fill' : 'ph-bookmark-simple'}`;
  ...
  card.querySelector('.card-like-btn').addEventListener('click', async e => {
    ...
    try {
      ...
      const icon = btn.querySelector('i');
      if (icon) icon.className = `ph ${res.liked ? 'ph-heart-fill' : 'ph-heart'}`;
      btn.classList.add('pulse');
      setTimeout(() => btn.classList.remove('pulse'), 400);
    } catch {
      showToast('Algo salió mal, intenta de nuevo', 'error');
...
async function openModal(id) {
  ...
  likeBtn.innerHTML = `<i class="ph ${isLiked ? 'ph-heart-fill' : 'ph-heart'}"></i>`;
  const saveBtn = document.getElementById('btn-modal-save');
  saveBtn.classList.toggle('saved', isSaved);
  saveBtn.innerHTML = `<i class="ph ${isSaved ? 'ph-bookmark-simple-fill' : 'ph-bookmark-simple'}"></i>`;
